...
Parser.prototype.generateBuffer = function(ctx) {
    if (this.options.readUntil === 'eof') {
        ctx.pushCode('{0} = buffer.slice(offset, buffer.length);',
            ctx.generateVariable(this.varName)
            );
    } else if (this.options.readUntil) {
        //TODO: check typeof readUntil, String type may need escaping
        ctx.pushCode('{0} = buffer.slice(offset, buffer.indexOf(\'{1}\', offset));',
            ctx.generateVariable(this.varName),
            this.options.readUntil
            );
        ctx.pushCode('offset += buffer.indexOf(\'{0}\', offset) - offset;', this.options.readUntil);
    } else {
